...
describe("WaitlistEntryRepository", () => {
  ...
  it("expires stale offers, including offers without an expiry, and invalidates their tokens", async () => {
    ...
    expect(mockPrismaClient.waitlistEntry.updateMany).toHaveBeenCalledWith({
      where: { status: "OFFERED", OR: [{ offerExpiresAt: { lt: now } }, { offerExpiresAt: null }] },
